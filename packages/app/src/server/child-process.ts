import { GameLoop, Runtime, World } from "@dungeon-crawler/runtime";
...
export const setupChildProcess = (app: expressWs.Application): void => {
  ...
  const wsServer = new WsServer(app, `/play/${process.env.ROOM_ID}`);
  Runtime.registerWsServer(wsServer);
  ...
  /**
   * When we close, we should close like this:
   * expressWsInstance
      .getWss()
      .clients.forEach((ws) =>
